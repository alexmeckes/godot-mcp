...
function schemaTypeLabel(schema: z.ZodTypeAny): string {
  ...
  if (schema instanceof z.ZodArray) {
    return `array<${schemaTypeLabel(schema.element as unknown as z.ZodTypeAny)}>`;
  }
  if (schema instanceof z.ZodRecord) return "record<string, unknown>";
  if (schema instanceof z.ZodObject) return "object";
  if (schema instanceof z.ZodUnion) {
    return schema.options
      .map((option) => schemaTypeLabel(option as unknown as z.ZodTypeAny))
      .join(" | ");
  }
  if (schema instanceof z.ZodTuple) {
    return `tuple<${schema.def.items
      .map((item) => schemaTypeLabel(item as unknown as z.ZodTypeAny))
      .join(", ")}>`;
  }
  return "unknown";
}
...
export async function loadAiBridgePluginFiles(): Promise<Record<string, string>> {
  ...
  const entries = await Promise.all(
    AI_BRIDGE_FILENAMES.map(async (filename) => {
      const content = await fs.readFile(path.join(bridgeDirectory, filename), "utf-8");
      return [`addons/godot_ai_bridge/${filename}`, content] as const;
    })
  );
